...
<!--
  ...
  *** SHARE-CLASS PORTFOLIO VARIANT ***
  ===================================================================

  HOW THIS DIFFERS FROM tpt_v7_export.xslt
  ========================================
  The default exporter reads the look-through positions from the FUND-level
  portfolio  Fund/FundDynamicData/Portfolios/Portfolio . This variant instead
  reads them from each SHARE CLASS's own portfolio
      Fund/SingleFund/ShareClasses/ShareClass/Portfolios/Portfolio
  so the holdings, NAV (col 5), valuation date (col 6) and cash ratio (col 9)
  are taken share-class by share-class rather than at fund level. A fund whose
  share classes carry no own Portfolio produces no rows (there is nothing to
  look through at share-class level). Everything else - the AssetMasterData
  join, the 152-column layout and all per-column logic - is identical.
...
<xsl:stylesheet version="2.0"
  ...
  <xsl:template match="/">
    ...
    <xsl:for-each select="$root/Funds/Fund">
      ...
      <!-- Fund issuer / QRT portfolio block (cols 115,116,117,122,123a). The
           fund "issuer" is the management / investment company; the custodian
           is the depositary bank. Both live under FundStaticData. When they are
           absent we fall back to ControlData/DataSupplier (the issuing house),
           preserving the previous behaviour for un-enriched files. -->
      <xsl:variable name="invco"     select="$fund/FundStaticData/InvestmentCompany"/>
      <xsl:variable name="custodian" select="$fund/FundStaticData/Custodian"/>
      <xsl:variable name="fundIssuerCode"      select="$invco/Identifiers/LEI"/>
      <xsl:variable name="fundIssuerName"      select="($invco/Name, $root/ControlData/DataSupplier/Name)[1]"/>
      <xsl:variable name="fundIssuerCountry"   select="($invco/Address/Country, $root/ControlData/DataSupplier/SystemCountry)[1]"/>
      <xsl:variable name="fundCustodianCountry" select="$custodian/Address/Country"/>

      <!-- Fund-level NAV (used only as a fallback for share classes that do not
           carry their own TotalNetAssetValue). -->
      <xsl:variable name="navFund"
      ...
      <!-- Cash accounts (AssetType 'AC') for the share-class cash ratio (col 9). -->
      <xsl:variable name="acUids" select="$root/AssetMasterData/Asset[AssetType='AC']/UniqueID"/>

      <!-- One full position block PER SHARE CLASS (ISIN), reading that share
           class's OWN portfolio. Share-class portfolios are mandatory here, so
           the '#fund' fallback simply yields an empty $sc / no positions. -->
      ...
      <xsl:for-each select="if (exists($scList)) then $scList else '#fund'">
        ...
        <!-- This share class's own look-through portfolio (the variant's source). -->
        <xsl:variable name="scPortfolio" select="$sc/Portfolios/Portfolio"/>

        <!-- NAV (col 5), valuation date (col 6) and cash ratio (col 9) taken at
             SHARE-CLASS level: the share class's own NAV / portfolio, with a
             fund-level fallback for NAV only. -->
        <xsl:variable name="nav"
                      select="(($sc/TotalAssetValues/TotalAssetValue/TotalNetAssetValue/Amount[@ccy=$pccy])[last()], $navFund)[1]"/>
        <xsl:variable name="navDate"
                      select="($scPortfolio/NavDate,
                               $sc/TotalAssetValues/TotalAssetValue/NavDate)[1]"/>
        <xsl:variable name="cashRatio"
                      select="sum($scPortfolio/Positions/Position[UniqueID = $acUids]/TotalPercentage) div 100"/>
      ...
      <xsl:for-each select="$scPortfolio/Positions/Position">
        ...
        <xsl:value-of select="string-join((
          ...
          f:csv('N'),                                      (: 11 complete SCR delivery: 'N' = the SCR contribution columns (97-105b) are not delivered :)
          ...
          f:csv($fundIssuerCode),                          (: 115 fund issuer code = InvestmentCompany LEI :)
          f:csv(if ($fundIssuerCode) then '1' else ''),    (: 116 fund issuer code type: 1 = LEI :)
          f:csv($fundIssuerName),                          (: 117 fund issuer name = InvestmentCompany (fallback DataSupplier) :)
          f:csv(''),                                       (: 118 fund issuer sector (NACE) - QRT-node only, no FundStaticData home :)
          f:csv(''),                                       (: 119 fund issuer group code - QRT-node only :)
          f:csv(''),                                       (: 120 fund issuer group code type - QRT-node only :)
          f:csv(''),                                       (: 121 fund issuer group name - QRT-node only :)
          f:csv($fundIssuerCountry),                       (: 122 fund issuer country = InvestmentCompany country (fallback DataSupplier) :)
          f:csv(''),                                       (: 123 fund CIC - QRT-node only :)
          f:csv($fundCustodianCountry),                    (: 123a fund custodian country = Custodian country :)
          f:csv(''),                                       (: 124 duration :)
